            ...
            for (int j = 0; j < 13; j++) {
                if (! labelStates[i][j]) {
                    cardsLabels[i][j] = new JLabel(
                        new ImageIcon (getClass().getResource ("images/blank.card.gif"))
                    );
                } else {
                    char color = '.';
                    ...
                    cardsLabels[i][j] = new JLabel(
                        new ImageIcon(getClass().getResource("images/cards/" + Card.getCharCard(j + 2) + color + ".gif"))
                    );
                }
    ...
    private JPanel createSelectedPanel(int nrOfRequestedCards, Card[] alreadySelectedCards)
    {
        ...
        for (int i = 0; i < nrOfRequestedCards; i++) {
            if (alreadySelectedCards == null) {
                selectedLabels[i] = new JLabel(new ImageIcon(getClass().getResource("images/blank.card.gif")));
            } else {
                selectedLabels[i] = new JLabel(
                    new ImageIcon(getClass().getResource("images/cards/" + alreadySelectedCards[i].toString() + ".gif"))
                );
            ...
            for (int i = 0; i < selectedStates.length; i++) {
                ...
                if (! selectedStates[i]) {
                    ...
                    selectedLabels[i].setIcon(
                        new ImageIcon(getClass().getResource("images/cards/" + newCard.toString() + ".gif"))
                    );
    ...
    private class SelectedLabelListener extends MouseAdapter
    {
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            cardsLabels[row][bigColumn].setIcon(
                new ImageIcon(getClass().getResource("images/cards/" + existingCard.toString() + ".gif"))
            );
